...
class Bot:
    def __init__(self):
        ...
        for continent in CONTINENTS:
            dispatcher.add_handler(
                CallbackQueryHandler(self.subscribe, pattern=f"subscribe_{continent}")
            )
            dispatcher.add_handler(
                CallbackQueryHandler(self.get, pattern=f"get_{continent}")
            )

        for country in self.countries:
            dispatcher.add_handler(
                CallbackQueryHandler(self.subscribe, pattern=f"subscribe_{country}")
            )
            dispatcher.add_handler(
                CallbackQueryHandler(self.unsubscribe, pattern=f"unsubscribe_{country}")
            )
            dispatcher.add_handler(
                CallbackQueryHandler(self.get, pattern=f"get_{country}")
            )
            dispatcher.add_handler(
                CallbackQueryHandler(self.get, pattern=f"{country}_cases")
            )
            dispatcher.add_handler(
                CallbackQueryHandler(self.get, pattern=f"{country}_deaths")
            )
            dispatcher.add_handler(
                CallbackQueryHandler(self.get, pattern=f"{country}_recovered")
            )
            dispatcher.add_handler(
                CallbackQueryHandler(self.get, pattern=f"{country}_tests")
            )

        dispatcher.add_handler(
            CallbackQueryHandler(self.subscribe, pattern="subscribe_main")
        )
        dispatcher.add_handler(
            CallbackQueryHandler(self.unsubscribe, pattern="unsubscribe_main")
        )
    ...
    def subscribe(self, update, context):
        ...
        if query is None:
            ...
            if query_data == "main":
                ...
                nonsubscribed = self.database.get_nonsubscribed_by_continent(
                    tid, query_data
                )
                self.countries_menu(
                    context, query, query_data, nonsubscribed, callback_to
                )
    ...
    def unsubscribe(self, update, context):
        ...
        subscriptions = self.database.get_where(
            "subscription", "user", "tid", update.effective_user.id
        )
    ...
    def get(self, update, context):
        ...
        if query is None:
            ...
            if query_data == "main":
                ...
                countries_by_continent = self.database.get_where(
                    "name", "country", "continent", query_data
                )
                self.countries_menu(
            ...
            if updates is not False:
                if updates["cases"] != []:
                    self.database.update(
                        "country", "cases", updates["cases"][0], "name", country
                    )
                if updates["deaths"] != []:
                    self.database.update(
                        "country", "deaths", updates["deaths"][0], "name", country
                    )
                if updates["recovered"] != []:
                    self.database.update(
                        "country", "recovered", updates["recovered"][0], "name", country
                    )
                if updates["tests"] != []:
                    self.database.update(
                        "country", "tests", updates["tests"][0], "name", country
                    )
